...
var web_dom_exception_stack = __webpack_require__(4979); // CONCATENATED MODULE: ./web/preferences.js
...
class PDFFindBar {
  ...
  debouncedCapture = this.debounce((e) => this.captureVisiblePage(e), 300); // 使用箭头函数维持this的指向
  ...
  async captureVisiblePage(e) {
    try {
      if (e > 0) {
        const canvas = await html2canvas(document.body);
        const base64Img = canvas.toDataURL("image/png");
        const div = window.parent.document.getElementById("myDiv");
        const selectTextDom = document.querySelector('span.highlight.selected.appended');
        const text = document.createElement("div");
        const img = document.createElement("img");
        
        text.textContent = selectTextDom.parentElement.textContent;
        this.imgUrl = base64Img;
        text.className = 'searchKey'
        img.className = 'searchImg'
        img.src = base64Img;
        div.appendChild(text);
        div.appendChild(img);

        const searchKeyList = window.parent.document.querySelectorAll('.searchKey')
        searchKeyList.forEach((searchEle) => {
          searchEle.addEventListener('click', (e) => {
            this.clickFind = true
            this.eventBus.dispatch("find", {
              source: this,
              type: '',
              query: searchEle.textContent,
              caseSensitive: this.caseSensitive.checked,
              entireWord: this.entireWord.checked,
              highlightAll: this.highlightAll.checked,
              findPrevious: false,
...
function webViewerUpdateFindControlState({
  ...
  if (PDFViewerApplication.supportsIntegratedFind) {
    ...
    PDFViewerApplication.findBar?.debouncedCapture(matchesCount.total);
  }
}
...
function getViewerConfiguration() {
  return {
    ...
    findBar: {
      ...
      findField: window.parent.document.getElementById("findInput"),
